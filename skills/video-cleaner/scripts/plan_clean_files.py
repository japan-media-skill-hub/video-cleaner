...
def parse_args():
    ...
    parser.add_argument(
        "--subtitle-extensions",
        default=".srt,.ass,.ssa,.vtt,.sub",
        help="字幕扩展名，逗号分隔 (默认: .srt,.ass,.ssa,.vtt,.sub)",
    )
    parser.add_argument(
        "--valid-keywords",
    ...
    parser.add_argument(
        "--temp-tail-globs",
        default="",
        help="临时下载文件尾缀通配，逗号分隔 (示例: *.115chrome_*,*.115chrome_*.cfg)",
    )
...
def parse_glob_list(value: str) -> list:
    return [item.strip().lower() for item in value.split(",") if item.strip()]


def matches_any_glob(filename: str, patterns: list) -> bool:
    name_lower = filename.lower()
    return any(fnmatch.fnmatch(name_lower, pattern) for pattern in patterns)
...
def classify_videos(
    ...
    try:
        for item in folder.iterdir():
            if not item.is_file():
                continue
            if item.suffix.lower() not in video_extensions and not matches_any_glob(
                item.name, temp_tail_globs
            ):
                continue
...
def is_keyword_series_file(filename: str, valid_keywords: set) -> bool:
    stem = Path(filename).stem.lower()
...
def get_subtitle_prefix(filename: str) -> str:
...
def is_subtitle_for_video(filename: str, large_videos: list, small_videos: list, subtitle_extensions: set) -> bool:
    ext = Path(filename).suffix.lower()
    if ext not in subtitle_extensions:
        return False

    prefix = get_subtitle_prefix(filename)
    if not prefix:
        return False

    video_stems = {v[1].lower() for v in large_videos + small_videos}
    video_base_names = {v[4].lower() for v in large_videos + small_videos}
    if prefix in video_stems or prefix in video_base_names:
        return True
    return False
...
def classify_file(
    ...
    if matches_any_glob(filename, temp_tail_globs):
        return 'DELETE'
    ...
    if ext in subtitle_extensions:
        if is_subtitle_for_video(filename, large_videos, small_videos, subtitle_extensions):
            return 'KEEP'
        return 'DELETE'
    ...
    if ext in image_extensions:
        ...
        if is_keyword_series_file(filename, valid_keywords):
            return 'KEEP'
...
def scan_folder_for_files(
    ...
    operations = []
    large_videos, small_videos = classify_videos(
        folder, video_extensions, min_size_mb, temp_tail_globs
    )
...
def main():
    ...
    subtitle_extensions = parse_comma_list(args.subtitle_extensions)
    valid_keywords = parse_comma_list(args.valid_keywords)
    temp_tail_globs = parse_glob_list(args.temp_tail_globs)
    output_dir = Path(args.output)

    print(f"Scanning: {root_path}")
    print(f"Valid keywords: {valid_keywords}")
    print(f"Subtitle extensions: {subtitle_extensions}")
    print(f"Temp tail globs: {temp_tail_globs}")
